...
export default function Play () {
    ...
    const handlePrevSong = () => {
        if (playMode === "random") {
            // 生成一个随机索引，但不包括当前歌曲的索引
            let randomIndex = Math.floor(Math.random() * (currentPlayList.length - 1));
            // 确保新索引不等于当前索引且在有效范围内
            while (randomIndex >= musicId) {
                randomIndex = Math.floor(Math.random() * (currentPlayList.length - 1));
            }
            setMusicId(randomIndex + 1);
        } else {
            // 正常逻辑
            if (musicId > 1) {
                setMusicId(musicId);
                ...
                // 循环到最后一首
                setMusicId(currentPlayList.length);
            }
        }
        song = currentPlayList[musicId];
        setCurrentPlayedTime(0);
        ...
        <div className="w-full fixed bottom-0">
            <div className="">
                ...
                <div className="w-full h-2 relative">
                    <div className="range h-2">
                        <input type="range" className='w-full h-2' defaultValue="0" ref={progressBar} onChange={changeRange} />
                    </div>
                </div>
            </div>
