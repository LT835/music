...
async function fetchData() {
  const response = await fetch('http://localhost:3000/api/trending/udio');
  const data = await response.json();
  return data.message;
}
export default async function TrendingUdio() {
  const title = "Trending";
  const trendingList = await fetchData();
  return (
    <div className={clsx('w-[100%] h-[100vh] pt-[122px] pl-[320px]')}>
      <ThirdNav title="Trending" first="/trending" second="/newset/udio" subtitle="Udio AI Songs" />
